...
*/
			...
			*/
			...
			if ( move_uploaded_file($file['tmp_name'], $new_file) ) {
				...
				if ( 'teams' == $mode )
					...
					$this->customImportTeams($new_file, $delimiter);
				elseif ( 'matches' == $mode )
					//$this->importMatches($new_file, $delimiter);
					$this->customImportMatches($new_file, $delimiter);
			} else {
	 ...
	 * @param string $delimiter
	 */
	function customImportTeams( $file, $delimiter )
	{
		global $leaguemanager;
		...
		if ($handle) {
			...
			while (!feof($handle)) {
				...
				if ( $i > 2 && $line ) {
					...
					// keine Duplikate
					if (!in_array($team, $teamsArr)) {
						array_push($teamsArr, $team);
						$custom = apply_filters( 'leaguemanager_import_teams_'.$league->sport, $custom, $line );
	 ...
	 * @param string $delimiter
	 */
	function customImportMatches( $file, $delimiter )
	{
		global $leaguemanager;
		...
		if ($handle) {
			...
			while (!feof($handle)) {
				...
				if ( $i > 2 && $line ) {
					$date = trim($line[2], '"');
					$day = "20".substr($date, -2)."-".substr($date, 3, 2)."-".substr($date, 0, 2);
					$date = ( !empty($line[3]) ) ? $day." ".$line[3] : $day. " 00:00";
					$season = $this->season = '13/14';
					$match_day = $i - 2;
					$date = trim($date);
					$location = trim($line[7],'"')." ".trim($line[9],'"');
					$home_team = $this->getTeamID(trim($line[5],'"'));
					$away_team = $this->getTeamID(trim($line[6],'"'));

					$match_id = $this->addMatch($date, $home_team, $away_team, $match_day, $location, $this->league_id, $season, '','', array());

					$matches[$match_id] = $match_id;
					$home_teams[$match_id] = $this->getTeamID(trim($line[5],'"'));
					$away_teams[$match_id] = $this->getTeamID(trim($line[6],'"'));
					$score = explode(":", "0:0");
					$home_points[$match_id] = "";$score[0];
					$away_points[$match_id] = "";$score[1];
					$custom = apply_filters( 'leaguemanager_import_matches_'.$league->sport, $custom, $line, $match_id );
		...
		if ( $matches ) {
	  	$league = $this->league;
			...
			foreach ( $matches AS $match ) {
				...
				$match->season."\t".
				$match->match_day."\t".
				$teams[$match->home_team]['title']."\t".
				$teams[$match->away_team]['title']."\t".
				$match->location."\t".
